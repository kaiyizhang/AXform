...
class Discriminator(nn.Module):
    ...
    def forward(self, x):  # B*2048*3
        x = x.transpose(1, 2).contiguous()
        x = F.relu(self.conv1(x))  # if r-gan uses vanilla gan，F.relu -> self.leaky_relu
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = F.relu(self.conv4(x))
        x = self.conv5(x)
        x, _ = torch.max(x, dim=2)  # B*512
...
class Runner:
    def __init__(self, opt):
        ...
        self.loss_gp = GradientPenalty(lambdaGP=10, gamma=1, device=opt.device)  # if r-gan uses vanilla gan, annotate this
    ...
    def train_model(self, data, args):
        ...
        if args[1] % 10 == 0:
            ...
            if self.opt.visual:
                plot_diff_pcds([ref[i] for i in range(5)]+[self.gen[i] for i in range(5)],
                                vis=self.vis,
...
if __name__ == "__main__":
    ...
    if opt.mode == "train":
        ...
        opt.save_path = os.path.join('./log', 'latent_3d_points/r-gan', now)
        if not os.path.exists(opt.save_path):
            os.makedirs(opt.save_path)
        os.system('cp ./models/latent_3d_points/r-gan.py %s' % opt.save_path)
